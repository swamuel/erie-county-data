...
def _filter_pois_by_type(pois, primary_category, type_val):
...
def render(merged_tract_2023, pantry_locations, pois, transit_stops):
    ...
    with col_map:
        ...
        # ── POI scatter layers ────────────────────────────────
        def add_scatter_layer(pts, color, radius=250):
            if len(pts) == 0:
                return
            layers.append(pdk.Layer(
                "ScatterplotLayer",
        ...
        if show_pantries and pantry_locations is not None and len(pantry_locations) > 0:
        ...
        if show_snap and pois is not None:
            ...
            snap_pts = pois[snap_flag].dropna(subset=["lat", "lon"])
            add_scatter_layer(snap_pts, [16, 185, 129, 220], 200)

        if show_hospitals and pois is not None:
            h = _filter_pois_by_type(pois, "Health", "Hospital")
            add_scatter_layer(h, [220, 38, 38, 240], 350)

        if show_clinics and pois is not None:
            c = _filter_pois_by_type(pois, "Health", "Clinic")
            add_scatter_layer(c, [251, 146, 60, 220], 250)

        if show_pharmacies and pois is not None:
            p = _filter_pois_by_type(pois, "Health", "Pharmacy")
            add_scatter_layer(p, [239, 68, 68, 200], 200)

        if show_libraries and pois is not None:
            lib = _filter_pois_by_type(pois, "Education & Civic", "Library")
            add_scatter_layer(lib, [59, 130, 246, 220], 250)

        if show_comm_centers and pois is not None:
            cc = _filter_pois_by_type(pois, "Civic & Social", "Community Center")
            add_scatter_layer(cc, [168, 85, 247, 220], 250)

        if show_social_svc and pois is not None:
            ss = _filter_pois_by_type(pois, "Civic & Social", "Social Services")
            add_scatter_layer(ss, [192, 132, 252, 200], 200)

        if show_transit and transit_stops is not None and len(transit_stops) > 0:
            stop_pts = transit_stops.dropna(subset=["stop_lat", "stop_lon"]).rename(
                columns={"stop_lat": "lat", "stop_lon": "lon"}
            )
            add_scatter_layer(stop_pts, [107, 114, 128, 180], 150)
